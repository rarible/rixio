...
import type { Wrapped, Lifted } from "@rixio/wrapped"
import { WrappedRejected, WrappedPending, toWrapped } from "@rixio/wrapped"
...
> {
  ...
  shouldComponentUpdate(nextProps: RProps): boolean {
    if (this.props !== nextProps) {
      ...
      const finalProps = this.extractProps(nextProps)
      this.doUnsubscribe(oldProps, finalProps)
      this.doSubscribe(oldProps, finalProps)
  ...
  private doUnsubscribe(oldProps: Lifted<P>, nextProps: Lifted<P>) {
    walk(oldProps, (value, lens) => {
      if (value instanceof Observable && lens.get(nextProps) !== value) {
  ...
  private handle(lens: Lens<Lifted<P>, any>, value: any) {
    const nextState = lens.set(value, this._state)
