...
import type { ValidationResult, ValidationResultError, ValidationResultSuccess, ValidationStatus } from "./domain"
...
describe("FormStore", () => {
  ...
  it("should display validating status while perform async validation (with Observable)", async () => {
    const atom = signUpForm.getAtom()
    const validate = (): Observable<ValidationResult<SignUpFormSchema>> => {
