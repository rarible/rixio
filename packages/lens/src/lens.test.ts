...
function roundtrip<T, U>(name: string, l: Lens<T, U>, obj: T, oldVal: U, nextValue: U) {
  describe(`lens roundtrip: ${name}`, () => {
    it("get", () => expect(l.get(obj)).toEqual(oldVal))
    it("set", () => expect(l.get(l.set(nextValue, obj))).toEqual(nextValue))
  })
}
...
function testLens<O, P>(name: string, l: Lens<O, P>, obj: O, currentValue: P, nextValue: P, nextValue2: P) {
  testLaws(l, obj, nextValue, nextValue2, name)
  roundtrip(name, l, obj, currentValue, nextValue)
}
...
describe("json", () => {
  ...
  describe("find", () => {
    ...
    it("set", () => expect(l.get(l.set(nextValue, xs))).toEqual(undefined))
  })
