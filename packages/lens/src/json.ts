...
import { structEq, setKey, conservatively, findIndex } from "./utils"
...
export function replaceImpl<T>(originalValue: T, nextValue: T): Lens<T, T> {
  return Lens.create<T, T>(
    x => (structEq(x, originalValue) ? nextValue : x),
    conservatively((y: T) => (structEq(y, nextValue) ? originalValue : y)),
  )
}
...
const indexCache = new SimpleCache<number, Prism<any[], any>>(i =>
  Prism.create(
    xs => xs[i],
    (v, xs) => {
      if (xs.length <= i) return xs.concat(new Array(i - xs.length), [v])
